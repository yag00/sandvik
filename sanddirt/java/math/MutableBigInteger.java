...
class MutableBigInteger {
    ...
    void addShifted(MutableBigInteger addend, int n) {
        ...
        while (x > 0 && y > 0) {
            ...
            y--;
            int bval =
                y + addend.offset < addend.value.length ? addend.value[y + addend.offset] : 0;
    ...
    void multiply(MutableBigInteger y, MutableBigInteger z) {
        ...
        for (int j = yLen - 1, k = yLen + xLen - 1; j >= 0; j--, k--) {
            long product =
                (y.value[j + y.offset] & LONG_MASK) * (value[xLen - 1 + offset] & LONG_MASK)
                + carry;
    ...
    MutableBigInteger divide(MutableBigInteger b,
    ...
    MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b,
        ...
        if (r < s) {
            ...
            int m =
                1 << (32 - Integer.numberOfLeadingZeros(s / BigInteger.BURNIKEL_ZIEGLER_THRESHOLD));

            int j = (s + m - 1) / m; // step 2a: j = ceil(s/m)
            int n = j * m; // step 2b: block length in 32-bit units
            long n32 = 32L * n; // block length in bits
            int sigma =
                (int) Math.max(0, n32 - b.bitLength()); // step 3: sigma = max{T | (2^T)*B < beta^n}
            ...
            MutableBigInteger a1 =
                aShifted.getBlock(t - 1, t, n); // the most significant block of a

            // step 7: z[t-2] = [a[t-1], a[t-2]]
            MutableBigInteger z =
                aShifted.getBlock(t - 2, t, n); // the second to most significant block
     ...
     * <br/>
     * {@code this} must be a nonnegative number such that {@code this.bitLength() <=
     * 2*b.bitLength()}
     ...
     * <br/>
     * {@code this} must be a nonnegative number such that {@code 2*this.bitLength() <=
     * 3*b.bitLength()}
     * @param quotient output parameter for {@code this/b}
     * @return {@code this%b}
     */
    ...
    private MutableBigInteger getBlock(int index, int numBlocks, int blockLength) {
        ...
        int[] newVal =
            Arrays.copyOfRange(value, offset + intLen - blockEnd, offset + intLen - blockStart);
    ...
    private static void copyAndShift(
        int[] src, int srcFrom, int srcLen, int[] dst, int dstFrom, int shift) {
        int n2 = 32 - shift;
        int c = src[srcFrom];
