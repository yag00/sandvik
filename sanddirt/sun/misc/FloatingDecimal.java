...
public class FloatingDecimal {
    ...
    private static final BinaryToASCIIConverter B2AC_NEGATIVE_INFINITY =
        new ExceptionalBinaryToASCIIBuffer("-" + INFINITY_REP, true);
    ...
    static class BinaryToASCIIBuffer implements BinaryToASCIIConverter {
        ...
        private void setSign(boolean isNegative) { this.isNegative = isNegative; }
        ...
        private void developLongDigits(int decExponent, long lvalue, int insignificantDigits) {
            if (insignificantDigits != 0) {
                ...
                long pow10 = FDBigInteger.LONG_5_POW[insignificantDigits]
                    << insignificantDigits; // 10^i == 5^i * 2^i;
        ...
        static int estimateDecExp(long fractBits, int binExp) {
            double d2 =
                Double.longBitsToDouble(EXP_ONE | (fractBits & DoubleConsts.SIGNIF_BIT_MASK));
            double d =
            ...
            if (exponent >= 0 && exponent < 52) { // hot path
                ...
                int r = (int) (((dBits & DoubleConsts.SIGNIF_BIT_MASK) | FRACT_HOB)
                               >> (EXP_SHIFT - exponent));
    ...
    static class ASCIIToBinaryBuffer implements ASCIIToBinaryConverter {
        ...
        public double doubleValue() {
            ...
            final int B5 = Math.max(
                0, -exp); // powers of 5 in bigB, value is not modified inside correctionLoop
            final int D5 = Math.max(
                0, exp); // powers of 5 in bigD, value is not modified inside correctionLoop
        ...
        correctionLoop:
            while (true) {
                ...
                if ((cmpResult) < 0) {
                    ...
                    if (ieeeBits == 0
                        || ieeeBits
                            == DoubleConsts.EXP_BIT_MASK) { // 0.0 or Double.POSITIVE_INFINITY
        ...
        private static final float[] SINGLE_SMALL_10_POW = {1.0e0f,
                                                            1.0e1f,
                                                            1.0e2f,
                                                            1.0e3f,
                                                            1.0e4f,
    ...
    private static BinaryToASCIIConverter getBinaryToASCIIConverter(float f) {
        ...
        // call the routine that actually does all the hard work.
        buf.dtoa(
            binExp, ((long) fractBits) << (EXP_SHIFT - SINGLE_EXP_SHIFT), nSignificantBits, true);
        return buf;
    }
    ...
    parseNumber:
        ...
        digitLoop:
            ...
            if (i < len
                && ((i != len - 1)
                    || (in.charAt(i) != 'f' && in.charAt(i) != 'F' && in.charAt(i) != 'd'
                        && in.charAt(i) != 'D'))) {
                break parseNumber; // go throw exception
            }
            if (isZero) {
    ...
    private static class HexFloatPattern {
        ...
        private static final Pattern VALUE = Pattern.compile(
            // 1           234                   56                7                   8      9
            "([-+])?0[xX](((\\p{XDigit}+)\\.?)|((\\p{XDigit}*)\\.(\\p{XDigit}+)))[pP]([-+])?(\\p{"
            + "Digit}+)[fFdD]?");
    }
        ...
        if (!validInput) {
            ...
            if (exponent >= FloatConsts.MIN_EXPONENT) {
                if (exponent > FloatConsts.MAX_EXPONENT) {
                    ...
                    int threshShift =
                        DoubleConsts.SIGNIFICAND_WIDTH - FloatConsts.SIGNIFICAND_WIDTH - 1;
                    boolean floatSticky =
                        (significand & ((1L << threshShift) - 1)) != 0 || round || sticky;
                    int iValue = (int) (significand >>> threshShift);
                    if ((iValue & 3) != 1 || floatSticky) {
                        iValue++;
                    }
                    floatBits |=
                        (((((int) exponent) + (FloatConsts.EXP_BIAS - 1))) << SINGLE_EXP_SHIFT)
                        + (iValue >> 1);
                }
            } else {
                if (exponent < FloatConsts.MIN_SUB_EXPONENT - 1) {
                    ...
                    int threshShift =
                        (int) ((DoubleConsts.SIGNIFICAND_WIDTH - 2 + FloatConsts.MIN_SUB_EXPONENT)
                               - exponent);
                    assert threshShift
                        >= DoubleConsts.SIGNIFICAND_WIDTH - FloatConsts.SIGNIFICAND_WIDTH;
            ...
            if (exponent > DoubleConsts.MAX_EXPONENT) { // Infinite result
                ...
                if (exponent <= DoubleConsts.MAX_EXPONENT && // (Usually) normal result
                    ...
                    if (exponent < (DoubleConsts.MIN_SUB_EXPONENT - 1)) {
                        ...
                        int bitsDiscarded =
                            53 - ((int) exponent - DoubleConsts.MIN_SUB_EXPONENT + 1);
